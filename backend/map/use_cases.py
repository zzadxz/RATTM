...
def get_user_all_locations_and_company(user_id):
    ...
    for transaction in user_transactions:
        company_name = _get_closest_match(
            transaction["merchant_name"], list(esg_data.keys())
        )
        print(company_name)
        if company_name:
            transact_map_data = {
                ...
                "merchant_percentile": _company_tier(
                    esg_data[company_name]["environment_score"]
                ),
            }
            ...
            transact_map_data = {
                "location": (transaction["latitude"], transaction["longitude"]),
                "merchant_name": transaction["merchant_name"],
                "merchant_percentile": "No environmental score available for "
